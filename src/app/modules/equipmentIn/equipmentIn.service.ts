...
// create
const create = async (data: EquipmentIn): Promise<EquipmentIn | null> => {
  const result = await prisma.equipmentIn.create({
    data,
  });
...
const updateSingle = async (
  ...
  if (!isExist) {
    throw new ApiError(httpStatus.NOT_FOUND, 'EquipmentIn Not Found');
  }
  ...
  if (!result) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Failed to Update Equipment In');
  }
...
// delete single
const deleteSingle = async (id: string): Promise<EquipmentIn | null> => {
  // check is exist
  const isExist = await prisma.equipmentIn.findUnique({
    where: {
      id,
  ...
  if (!isExist) {
    throw new ApiError(httpStatus.NOT_FOUND, 'Equipment In Not Found');
  }
